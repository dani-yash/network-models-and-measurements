...
# A. Graph Model Generators
    ...
    # Creating the graph.
    ErdosRenyiGraph = nx.gnp_random_graph(n, p)

    # Finding the largest connected component.
    largest_cc = max(nx.connected_components(ErdosRenyiGraph), key=len)

    # Making a copy of the graph that only has the greatest connected component.
    GCC = ErdosRenyiGraph.subgraph(largest_cc).copy()

    # Basic graph parameters.
    print("The n value for er", effective_run, end=" ")
    print("was: ", n,)
    print("The p value for er", effective_run, end=" ")
    print("was: ", p)
    print("For er", effective_run, end=", ")
    ...
    plt.savefig('ER{}.png'.format(effective_run), dpi=600)
...
# B. Degree Distribution Plot
    ...
    # Saving the graph as an image.
    plt.savefig('ER{}DD.png'.format(effective_run), dpi=600)
...
# B. Clustering Plot
    ...
    # Saving the graph as an image.
    plt.savefig('ER{}CP.png'.format(effective_run), dpi=600)
